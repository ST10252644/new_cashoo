...
interface RewardDao {
    ...
    // Inserts a reward into the database or replaces it if there's a conflict (e.g. duplicate primary key)
    @Insert(onConflict = OnConflictStrategy.REPLACE)//(Viegen, 2022), (androidbyexample, 2024)
    suspend fun insertReward(reward: Reward)

    // Retrieves a specific reward by its ID as a Flow (for observing changes)
    @Query("SELECT * FROM Reward WHERE rewardID = :id")
    fun getRewardById(id: Int): Flow<Reward?>

    // Retrieves all rewards from the database as a Flow
    ...
    // Deletes a specific reward from the database
    @Delete
    suspend fun deleteReward(reward: Reward)

    // Deletes all entries from the Reward table
