...
@Dao
interface SubCategoryDao {//(Viegen, 2022), (androidbyexample, 2024)

    // Inserts a subcategory into the database or replaces it if a conflict occurs (e.g. duplicate primary key)
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertSubCategory(subCategory: SubCategory)

    // Retrieves a specific subcategory by its ID as a Flow
    @Query("SELECT * FROM SubCategory WHERE subCategoryID = :id")
    fun getSubCategoryById(id: Int): Flow<SubCategory?>

    // Retrieves all subcategories from the database as a Flow
    @Query("SELECT * FROM SubCategory")
    fun getAllSubCategories(): Flow<List<SubCategory>>

    // Deletes a specific subcategory entry from the database
    @Delete
    suspend fun deleteSubCategory(subCategory: SubCategory)

    // Deletes all records from the SubCategory table (non-suspending version)
    @Query("DELETE FROM SubCategory")
    fun deleteAllTableName()

    // Retrieves all subcategories associated with a specific user ID
    @Query("SELECT * FROM SubCategory WHERE userId = :userId")
    suspend fun getSubCategoriesForUser(userId: Int): List<SubCategory>

    // Deletes all subcategories from the table (suspending version)
    @Query("DELETE FROM SubCategory")
