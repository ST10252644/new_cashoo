...
interface CategoryDao {
    ...
    // Insert a single category into the database; replaces it if there's a conflict (e.g. duplicate primary key)
    @Insert(onConflict = OnConflictStrategy.REPLACE)//(Viegen, 2022), (androidbyexample, 2024)
    suspend fun insertCategory(category: Category)

    // Retrieve all categories as LiveData for observing changes in real-time
    @Query("SELECT * FROM Category")
    fun getAllCategories(): LiveData<List<Category>>

    // Insert a list of categories in bulk
    @Insert
    suspend fun insertCategories(categories: List<Category>)

    // Get all categories belonging to a specific user (based on userId)
    @Query("SELECT * FROM Category WHERE userId = :userId")
    suspend fun getCategoriesForUser(userId: Int): List<Category>

    // Retrieve a single category by its ID as a Flow
    ...
    // Retrieve all categories for a specific user as LiveData
    @Query("SELECT * FROM Category WHERE userId = :userId")
    fun getCategoriesByUser(userId: Int): LiveData<List<Category>>
}//(Cal, 202 // 3), (College, 2025)
