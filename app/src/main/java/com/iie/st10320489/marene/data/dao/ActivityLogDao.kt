...
@Dao
interface ActivityLogDao {//(Viegen, 2022), (androidbyexample, 2024)

    // Inserts a new activity log or replaces it if there's a conflict (e.g. same ID)
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertLog(log: ActivityLog)

    // Retrieves a specific activity log by its unique ID as a Flow (for real-time updates)
    @Query("SELECT * FROM ActivityLog WHERE logID = :id")
    fun getLogById(id: Int): Flow<ActivityLog?>

    // Retrieves all activity logs from the database as a Flow
    ...
    // Deletes all entries from the ActivityLog table
