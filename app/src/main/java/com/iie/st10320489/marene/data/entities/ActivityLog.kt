...
// Room entity representing an activity log entry performed by a user
@Entity( // ((Viegen, 2022), (androidbyexample, 2024))
    tableName = "ActivityLog",
    foreignKeys = [ForeignKey(
        entity = User::class,        // Establish a (foreign key) relationship with the User table
        parentColumns = ["userId"],      // Reference the userId in the User table
        childColumns = ["userId"]        // Link it to the userId in this table
    )]
)
data class ActivityLog(
    @PrimaryKey(autoGenerate = true) val logId: Int = 0,  // Auto-generated primary key for the log entry
    val userId: Int,              // ID of the user who performed the action
    val action: String,
    val timestamp: String,        // Time the action occurred (e.g., "2024-10-01 14:33")
    val details: String?          // Optional additional details about the action
)
